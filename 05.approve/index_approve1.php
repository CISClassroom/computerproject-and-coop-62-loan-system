<?php                                           //---------------------- ผู้อนุมัติ CA นายสุนย์ทร อรรคแสง --------------------//
  ...
  if( isset($_SESSION['approve_username']) AND !empty($_SESSION['approve_password'])):
    ...
    try
    {
        $sql_1 = "SELECT * FROM `petition` WHERE p_status='อนุมัติเรียบร้อย' AND p_position = 'CallCenter' ORDER BY p_code ASC";
        $stmt_1 = $dbh->prepare($sql_1);
        $stmt_1->execute();

        $count1 = "SELECT count(*) FROM `petition` WHERE p_status='อนุมัติเรียบร้อย'AND p_position = 'CallCenter'";
            ...
            <thead>
              ...
              <tbody>
                ...
                <td><center><?=$i++?></center></td>
                  ...
                  <td>
                    <center>
                        <a href="project_detail4.php?p_code=<?php echo $row['p_code']; ?>" class="btn btn-primary">รายละเอียด</a>
                    </center>
                  </td>
                </tr>
                <?php };
