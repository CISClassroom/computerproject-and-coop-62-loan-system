<?php                                           //---------------------- ผู้อนุมัติ CA นายสุนย์ทร อรรคแสง --------------------//
  ...
  if( isset($_SESSION['approve_username']) AND !empty($_SESSION['approve_password'])):
    ...
    $SS = $_SESSION["Personnel_code"];
    $SN = $_SESSION['Personnel_sign'];
    
    try
    {
      $sql_1 = "SELECT * FROM `petition` WHERE p_status='รอดำเนินการ' AND p_position='CallCenter' ORDER BY p_code ASC";
      $stmt_1 = $dbh->prepare($sql_1);
      $stmt_1->execute();

      $count1 = "SELECT count(*) FROM `petition` WHERE p_status='รอดำเนินการ'AND p_position='CallCenter'";
            ...
            <thead>
              ...
              <tbody>
                ...
                <td><center><?=$i++?></center></td>
                  ...
                  <td><center><?php echo $row['p_position']; ?>
                  <td><center><?php echo $row['p_date_borrow'] ?></center></td>
                  <td><center><?php echo $row['p_amount'] ?></center></td>
                  <td><b><center><font color='blue'><?php echo $row['p_status'] ?></span></center></b></td>
                  <td>
                    <center>
                      <a href="project_detail3.php?p_code=<?php echo $row['p_code']; ?>" class="btn btn-warning">รายละเอียด</a>
                    </center>
                  </td>
                </tr>
                <?php };
