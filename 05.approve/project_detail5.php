<?php
  ...
  if( isset($_SESSION['approve_username']) AND !empty($_SESSION['approve_password'])):
    ...
    try
    {
      ...
      $sql_3 = "SELECT * FROM `personnel` WHERE Personnel_FL_Name = '$SA'";
      $stmt_3 = $dbh->prepare($sql_3);
      $stmt_3->execute();
      $row3 = $stmt_3->fetch(PDO::FETCH_ASSOC);
...
<html lang="en">
  ...
  <body>
    ...
    <div class="shadow-lg p-3 mb-5 bg-white rounded" id="admin">
        ...
        <form method="post" action="update_project_process5.php" name='form1' class="needs-validation" novalidate>
        ...
        <input type="hidden"  name="p_code" value="<?php echo $row['p_code']; ?>"readonly/>
        <input type="hidden"  name="m_name" value="<?php echo $SA; ?>"readonly/>
        <div class="text-left">
        ...
        <p><span style="padding-right:70px">(<?php echo $row['p_signature2']; ?>)</p>
	      <p><span style="padding-right:70px">ผู้กู้ยืมเงิน</p><br><br>
        </div>

        <div class="text-right"> 
        <p><span style="padding-right:30px">ลงชื่อ : <img src="..\signature\sign_image\<?php echo $row3['Personnel_sign'] ?>" width="170" alt="">
        <p><span style="padding-right:70px">(<?php echo $SA; ?>)</p>
	      <p><span style="padding-right:70px">ผู้อนุมันติให้กู้</p>
        </div>
        <hr><br>

        <div class="text-center">
        <button type="submit" class="btn btn-success" name="submit" value="Edit Now" 
            onclick="if(confirm('ต้องการยื่นยันการอนุมัติ ยืนยัน?')) return true; else return false;">อนุมัติการขอยืมเงิน</button>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <a class="btn btn-danger" href="\tot\05.approve\index_approve2.php" role="button" 
            onclick="if(confirm('ต้องการย้อนกลับ ย้อนกลับ?')) return true; else return false;">ย้อนกลับ
            </a>
            </div>
            </div>
            </div>
