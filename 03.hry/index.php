<?php
  ...
  if( isset($_SESSION['Account_username']) AND !empty($_SESSION['Account_password'])):
    ...
    try
    {
      ...
      $sql_1 = "SELECT * FROM `petition` WHERE p_status='อนุมัติเรียบร้อย'  ORDER BY p_code ASC";
      $stmt_1 = $dbh->prepare($sql_1);
      $stmt_1->execute();

      $count1 = "SELECT count(*) FROM `petition` WHERE  p_status='อนุมัติเรียบร้อย'";
...
<html lang="en">
  ...
  <?php require(dirname(__FILE__).'/../main_menu/hry/hry_menubar.php');?>
    ...
    <div class="container">
      ...
      <h3 style="text-align: center;">รายละเอียดการกู้ยืมที่ได้รับการอนุมัติแล้ว</h3>
          <p class="lead">รายการที่กู้ยืมใหม่ จำนวน <span class="badge badge-success"><?php echo $count_Project ?></span> คน 
          <p class="lead">จำนวนเงินคงเหลือ จำนวน <span class="badge badge-success"><?php echo $money['M_money'] ?></span> บาท 
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <center>
                    <center>
                      <a href="update_hry.php?p_code=<?php echo $row['p_code'];?>" class="btn btn-warning">แก้ไข</a>
                    </center>
                    </center>
                  </td>
                </tr>
